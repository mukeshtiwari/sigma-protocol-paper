...
We also attempted the client-side benchmark on the WebAssembly path, compiling the same certified ballot-construction functions with CertiRocq at identical parameters, but the resulting code runs out of memory: the current verified WebAssembly backend computes directly on Rocq's binary-represented integers without garbage collection, and a single 2048-bit modular exponentiation already exhausts the 2\,GB WebAssembly address space. Running certified clients in the browser at real-world parameters therefore awaits primitive big-integer arithmetic in the verified backend; the extracted OCaml client is unaffected, as extraction maps Rocq integers to GMP-backed arbitrary-precision arithmetic.

Finally, we compiled the same certified ballot-construction term through a third pipeline: the Peregrine CakeML backend\footnote{\url{https://github.com/peregrine-project/cakeml-backend}} combined with the verified CakeML compiler~\cite{10.1145/2535838.2535841}. This is the strongest assurance point of our deployment paths: verified erasure, a backend accompanied by correctness proofs, and a fully verified compiler down to ARM64 machine code, with no unverified extraction and no unverified compiler anywhere in the chain. Like the WebAssembly backend, it computes on constructor-represented integers, but native code and a garbage collector let the computation complete: the full seven-candidate ballot takes 49.3 minutes (Table~\ref{tab:perf}). The three pipelines thus chart the current assurance--performance trade-off for certified cryptographic code: unverified extraction with GMP-backed arithmetic is fast, fully verified compilation runs but is four to five orders of magnitude slower, and the gap is attributable entirely to integer representation. 
