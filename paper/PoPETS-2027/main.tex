...
\begin{table}
  ...
  \begin{tabular}{@{}llr@{}}
    ...
    %% TODO(authors): once approval.wasm is regenerated from
    %% HeliosFrontendIns.v (2048-bit parameters) via CertiCoq-Wasm,
    %% add the browser-side row:
    %% Voting client (WebAssembly, browser) & $n = 7$ & [TBD]\\
    \bottomrule
  \end{tabular}
\end{table}

Table~\ref{tab:perf} summarises the results. For the voting client, the relevant metric is not raw throughput but perceived latency: a voter encrypts a single ballot, so encryption and proof generation must complete within interactive time bounds on commodity hardware. Encrypting a seven-candidate approval ballot and generating all seven disjunctive encryption proofs takes 62\,ms with our certified code---more than five times faster than the hand-written JavaScript shipped with the deployed Helios voting booth (334\,ms) on identical parameters. Certified code is therefore not synonymous with slow code: although our only optimisation is an efficient integer representation, the extracted OCaml outperforms a widely deployed hand-written implementation while carrying machine-checked correctness guarantees. We do not expect to match hand-optimised cryptographic libraries such as HACL$^{*}$~\cite{10.1145/3133956.3134043}, which occupy a different point in the assurance--performance space. For the verifier, the relevant metric is the end-to-end wall-clock time to validate an entire election, an offline batch task run by scrutineers: our certified verifier checks the complete IACR 2023 election (848 ballots, six candidates) in 39 seconds and the IACR 2024 election (932 ballots, seven candidates) in 51 seconds, including parsing the 21--26\,MB bulletin-board data, verifying every ballot's encryption proofs, homomorphically aggregating the valid ballots, and checking the trustees' decryption proofs and proofs of knowledge.
...
Our formalisation lacks mix-net~\cite{10.1145/358549.358563}, a key primitive in electronic voting that breaks the link between voters and their ballots by taking encrypted ballots and outputting them in a randomised order. Since outputs remain encrypted, a malicious mix-net could replace genuine ballots with forged ones, so it must provide a zero-knowledge proof of correct shuffling~\cite{10.1007/978-3-642-02620-1_28,10.1007/978-3-642-29011-4_17}. Mix-net is also used to verify ballot validity in the Schulze voting method \cite{haines2019verifiable}. Another possibility is to formalise Bulletproofs \cite{bunz2018bulletproofs}; this would allow us to provide compact range proofs for ballot validity (e.g., proving that a vote is $0$ or $1$), and could also be adapted to construct verifiable shuffles with logarithmic proof size (we have encoded the inner-product protocol but have not yet proved any security properties about our encoding). Another promising direction is a domain-specific language (DSL) for $\Sigma$-protocols embedded in Rocq. The \texttt{\seqsplit{sigma-compiler}}~\cite{sigma-compiler,cryptoeprint:2026/794} macro for Rust allows a protocol designer to write a high-level, declarative specification---lists of public parameters, secret witnesses, and logical statements---and automatically synthesises the corresponding prover and verifier functions. While this eliminates many manual implementation errors, the generated code itself is not formally verified. By embedding a similar DSL within Rocq, one could obtain a correct-by-construction pipeline: the same specification that is used to generate executable OCaml or Rust code could also serve as the input to a formalisation that proves, inside Rocq, that the resulting proof satisfies completeness, special soundness, and honest-verifier zero-knowledge. Given our current infrastructure, it would be fairly straightforward to  encode these algorithms and prove their security properties. 
   ...
   A natural question is whether our representation of probability
   distributions would scale to such a development. For the protocols in this
   paper, all distributions are finite and the zero-knowledge property is
   information-theoretic: the real and simulated transcript distributions
   coincide exactly, which is why representing a distribution as a list of
   outcome--probability pairs is both simple and sufficient. Reasoning in the
   random oracle model, by contrast, requires computational
   notions---negligible functions, adversary advantage, and
   indistinguishability---that our framework deliberately does not capture.
   We see two viable paths: extending our distribution monad with an
   approximate equivalence indexed by a security parameter, or interfacing
   with an existing framework for computational reasoning in Rocq, such as
   SSProve~\cite{10.1145/3594735}, while retaining our library as the source
   of extractable computational content. Importantly, the Fiat-Shamir
   transform changes only how challenges are produced, not the relation
   between transcripts: our certified provers and verifiers already take the
   challenge as an explicit input, so a non-interactive wrapper can reuse the
   extracted code unchanged, and only the security argument---not the
   code---requires the random oracle model.
