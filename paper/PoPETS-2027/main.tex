...
%% Packages needed for the Rocq listings and math in the body.
\usepackage{listings}
\usepackage{lstcoq}
\usepackage{amsmath}
\usepackage{seqsplit}
...
\begin{itemize}
  \item \textbf{A comprehensive certified library of $\Sigma$-protocols.} We formalise the Schnorr, Okamoto, and Chaum-Pedersen protocols, ElGamal encryption and decryption proofs, and proofs of linear relations over Pedersen commitments, together with generic $n$-ary Parallel, And, Or, Eq, and Neq compositions, and we prove completeness, special soundness, and special honest-verifier zero-knowledge for every construction (Section~\ref{schn_proof}).
  \item \textbf{Reusable proof infrastructure.} Our development rests on an abstract vector-space layer that cleanly separates group elements from field elements, and on a lightweight, list-based probability monad for reasoning about uniform distributions. This infrastructure is what makes the $n$-ary compositions and their information-theoretic zero-knowledge proofs tractable, and it repeatedly surfaced hidden assumptions: for example, our special soundness theorem for the Neq composition replaces the customary assumption of independent generators with an explicit, unconditional dichotomy (Section~\ref{schn_proof}).
  \item \textbf{One proof, two certified implementations.} To the best of our knowledge, ours is the first $\Sigma$-protocol formalisation that extracts to both OCaml (for the backend) and WebAssembly (for the frontend) from a single set of proofs. The WebAssembly path goes through CertiCoq-Wasm, a compiler that is itself verified in Rocq, so semantics preservation is machine-checked all the way down to the WebAssembly code (Section~\ref{coq_theorem}). This pipeline is enabled by Rocq's constructive logic; developments in classical frameworks such as Isabelle/HOL do not directly support extraction of executable code.
  \item \textbf{Real-world case studies.} We implement a complete client--server system for Approval Voting and a certified Helios verifier, which we run on real bulletin-board data from the IACR 2023 and 2024 elections (Section~\ref{case_studies}).
\end{itemize}
...
Section \ref{case_studies} demonstrates the practical use of the library through an Approval Voting client-server workflow and a certified Helios election verifier for IACR data, and reports performance measurements for the extracted code. Finally, Section \ref{rel_work} discusses related work, current limitations, and directions for future research.
...
\subsection{Reused Components}\label{reused_comp}
Our development builds on existing formal infrastructure where possible: the primality of the Helios parameters is established with the Coqprime library~\cite{thery2007primality}, and the WebAssembly backend is provided by CertiRocq~\cite{anand2017certicoq,meier2025certicoq}. The remaining components---the algebraic hierarchy of groups, fields, and vector spaces, the probability monad (Section~\ref{uniform_dist}), the efficient modular arithmetic for Schnorr groups and prime fields, and all protocol definitions and security proofs---were developed from scratch. This choice was deliberate: our design goal of extracting efficient executable code requires constructive definitions with a small extraction footprint, which precluded the reuse of existing proof-oriented libraries whose definitions are not geared towards computation.

\subsection{Trusted Computing Base}\label{tcb}
It is important to be precise about which components of our pipeline are formally verified and which must be trusted. On the verified side are: all protocol-level definitions together with their machine-checked proofs of completeness, special soundness, and special honest-verifier zero-knowledge; the modular arithmetic for the Schnorr group and prime field, including the Coqprime primality proofs of the 2048-bit and 256-bit Helios parameters; and, on the frontend path, the compilation from Rocq to WebAssembly, since CertiCoq-Wasm is itself implemented and verified in Rocq against the mechanised WebAssembly semantics. The trusted side comprises: the Rocq kernel (as in any Rocq development); the standard extraction mechanism to OCaml, which is not verified, together with the OCaml compiler and runtime; the WebAssembly engine of the browser that executes the extracted voting client; the unverified parser that reads bulletin-board data in our Helios verifier; and the source of randomness---our certified functions take random field elements as explicit inputs, so sampling this randomness happens outside the verified core, and the usual caveats about cryptographic random number generation apply. This trusted computing base is comparable to that of prior extraction-based developments~\cite{287095}, with the notable difference that on the WebAssembly path the unverified extraction step is replaced by a verified compiler, extending the machine-checked guarantees from the proof assistant down to the deployed code.
...
\subsection{Uniform Distribution}\label{uniform_dist}
...
\prod_{i=1}^{n} g_i^{x_{2,i}}$ are information-theoretically indistinguishable. The proof \texttt{\seqsplit{generalised\_okamoto\_witness\_indistinguishable}}, shown in Listing~\ref{wit_ind} in Appendix~\ref{appendix_listings}, relies on two 
...
\subsection{Efficient Modular Arithmetic}\label{eff_mod_arith}
As stated earlier, to ensure the ease of proving and modularity, we worked with an (abstract) vector space. However, to execute the code of our formalisation, we need to instantiate the vector space with a concrete entity. We have therefore encoded the Schnorr group \cite{schnorr1991efficient} (the multiplicative group of integers modulo a large prime $p$) and a prime field in the Rocq theorem prover; the Rocq definitions are shown in Listings~\ref{group_def} and~\ref{field_def} in Appendix~\ref{appendix_listings}. To encode the Schnorr group, we postulate three integers $k$, $p$, and $q$, assume that $p$ and $q$ are primes with $p = k q + 1$, encode the neutral element, the group multiplication, and the group inverse, and prove that our encoding is a commutative group. To encode the prime field, we represent $\mathbb{Z}_p$ as a record containing an integer $v$ together with a proof that it lies in the range $[0 \ldots p)$, define all field operations, and prove that they satisfy the field axioms. Finally, we combine the Schnorr group and the prime field into a vector space (Listing~\ref{vec_space} in Appendix~\ref{appendix_listings}) by defining the scalar multiplication \texttt{pow}, which takes an element of the Schnorr group and an element of the prime field and returns an element of the Schnorr group. The reader can see that our encoding eliminates all possibilities of mixing group and field elements, both represented as integers modulo some large prime \cite{10.1007/978-3-662-63958-0_24}. The source code for this section can be accessed in the file \href{https://anonymous.4open.science/r/SigmaProtocol-EBA5/src/Utility/Zpstar.v}{\textcolor{blue}{Zpstar.v}}.

\subsection{Efficient Extractor}\label{eff_extractor}
In this section, we explain the key ideas underlying the formalisation of an efficient extractor and simulator. Recall that for a $\Sigma$-protocol, the extractor is required to be efficient, i.e., it runs in time polynomial in the size of its input. However, functions defined over vector spaces do not capture computational complexity; in particular, they provide no information about the number of computational steps required to evaluate them on a given input. One possible approach in our formalisation would have been to adopt a monadic style~\cite{10.1145/3473585,10.1145/3158124}, where each function additionally returns the number of steps taken during its execution. This approach has the advantage of compositionality: given two monadic functions, their composition naturally yields the complexity of the combined computation. However, during code extraction, this additional bookkeeping would propagate through all functions, significantly complicating and effectively degrading the intended API. Instead, we adopt a more targeted approach. In practice, an extractor over a multiplicative group relies on the ability to efficiently compute inverses. In our concrete model, inverse is implemented via binary exponentiation using the function \texttt{\seqsplit{repeat\_op\_ntimes\_rec}}. We therefore introduce an inductive data type \texttt{\seqsplit{complexity\_repeat\_op\_ntimes\_rec}}, shown in Listing~\ref{extractor_complex} in Appendix~\ref{appendix_listings}, that captures the number of steps performed by this binary exponentiation procedure. Finally, we prove in theorem \texttt{\seqsplit{correct\_complexity\_repeat\_op\_ntimes\_rec}} that the function \texttt{\seqsplit{repeat\_op\_ntimes\_rec}} runs in logarithmic time in the value of its input, equivalently linear in the bit-length of the input. This approach allows us to preserve a clean API without introducing monadic overhead. While it lacks the compositional advantages of the monadic approach, it is sufficient for our purposes, as our primary objective is to extract executable (OCaml and WebAssembly) code whose APIs closely match those of real-world cryptographic libraries. The source code for this section can be access in the \href{https://anonymous.4open.science/r/SigmaProtocol-EBA5/src/Utility/Functions.v}{\textcolor{blue}{Functions.v}} file.




Two aspects of this design are worth spelling out, because the interaction between cost accounting and code extraction is subtle. First, the relation \texttt{\seqsplit{complexity\_repeat\_op\_ntimes\_rec}} lives in \texttt{Prop}, and Rocq's extraction erases all propositional content: the cost analysis therefore leaves no footprint whatsoever in the extracted OCaml or WebAssembly code. The extracted binary exponentiation is exactly the function one would write by hand, while the logarithmic bound on its running time remains machine-checked inside Rocq. A monadic encoding, by contrast, would change the type of every instrumented function from $A \rightarrow B$ to $A \rightarrow B \times \mathbb{N}$, and this instrumentation would survive extraction, forcing every caller---including external OCaml and JavaScript code interacting with our library---to thread and discard step counts. Second, the approach generalises: any function on the extraction path can be paired, after the fact, with an inductive cost relation and a correctness theorem connecting the two, without modifying the function or its API. The price is that cost proofs are per-function rather than compositional; we consider this an acceptable trade-off because only a handful of operations (modular exponentiation and inversion) dominate the running time of both the extractor and the simulator, and these are precisely the operations we analyse.
...
\subsection{Performance Evaluation}\label{perf_eval}
To assess whether the extracted code is practical, we measure both ends of the pipeline: ballot construction by the voting client and the Helios verifier on real election data. All measurements were taken on an Apple M3 Pro laptop (12 cores, 36\,GB RAM). The certified code is extracted to OCaml~5.4.1 and performs arbitrary-precision arithmetic through \texttt{zarith} (backed by GMP); the JavaScript baseline runs the exact cryptographic scripts served by the deployed Helios voting booth (\texttt{jsbn} big-integer arithmetic) under Node.js~26, on the same machine, with the same 2048-bit election parameters.

\begin{table}
  \caption{Performance of the certified code at the Helios parameters (2048-bit $p$, 256-bit $q$). Client-side numbers are for a single approval ballot with $n = 7$ candidates; verifier numbers are for complete IACR election data sets.}
  \label{tab:perf}
  \small
  \begin{tabular}{@{}llr@{}}
    \toprule
    ...
    \midrule
    Ballot encryption + proofs (certified OCaml) & $n = 7$ & 62\,ms\\
    Ballot encryption + proofs (Helios booth JS) & $n = 7$ & 334\,ms\\
    Ballot encryption + proofs (certified CakeML) & $n = 7$ & 49.3\,min\\
    Ballot verification (certified OCaml) & $n = 7$ & 49\,ms\\
    Ballot verification (Helios booth JS) & $n = 7$ & 385\,ms\\
    Helios verifier, IACR 2023 (certified OCaml) & 848 ballots & 39.2\,s\\
    Helios verifier, IACR 2024 (certified OCaml) & 932 ballots & 50.8\,s\\
    %% No WebAssembly row: at 2048-bit parameters the current verified
    %% backend runs out of linear memory (see the last paragraph of this
    %% section and bench/README.md in the artifact).
    \bottomrule
  \end{tabular}
\end{table}

Table~\ref{tab:perf} summarises the results. Encrypting a seven-candidate approval ballot and generating all seven disjunctive encryption proofs takes 62\,ms with our certified code, more than five times faster than the hand-written JavaScript shipped with the deployed Helios voting booth (334\,ms) on identical parameters. Certified code is therefore not synonymous with slow code: although our only optimisation is an efficient integer representation, the extracted OCaml outperforms a widely deployed hand-written implementation while carrying machine-checked correctness guarantees. We do not expect to match hand-optimised cryptographic libraries such as HACL$^{*}$~\cite{10.1145/3133956.3134043}, which occupy a different point in the assurance--performance space. Our certified verifier checks the complete IACR 2023 election (848 ballots, six candidates) in 39 seconds and the IACR 2024 election (932 ballots, seven candidates) in 51 seconds, verifying every ballot's encryption proofs, homomorphically aggregating the valid ballots, and checking the trustees' decryption proofs and proofs of knowledge.

We also attempted the client-side benchmark on the WebAssembly path, compiling the same certified ballot-construction functions with CertiRocq at identical parameters. The current verified WebAssembly backend computes directly on Rocq's binary-represented integers and allocates without garbage collection: we measured roughly 0.5 seconds and 50\,MB of allocation per 2048-bit modular multiplication, so a single modular exponentiation already exceeds the 2\,GB WebAssembly address space. Running certified clients in the browser at real-world parameters therefore awaits primitive big-integer arithmetic in the verified backend, which we consider an important direction for future work; the extracted OCaml client is unaffected, as extraction maps Rocq integers to GMP-backed arbitrary-precision arithmetic.

Finally, we compiled the same certified ballot-construction term through a third pipeline: the Peregrine CakeML backend\footnote{\url{https://github.com/peregrine-project/cakeml-backend}} combined with the verified CakeML compiler~\cite{10.1145/2535838.2535841}. This is the strongest assurance point of our deployment paths: verified erasure, a backend accompanied by correctness proofs, and a fully verified compiler down to ARM64 machine code, with no unverified extraction and no unverified compiler anywhere in the chain. Like the WebAssembly backend, it computes on constructor-represented integers, but native code and a garbage collector let the computation complete: the full seven-candidate ballot takes 49.3 minutes (Table~\ref{tab:perf}). The three pipelines thus chart the current assurance--performance trade-off for certified cryptographic code: unverified extraction with GMP-backed arithmetic is fast, fully verified compilation runs but is four to five orders of magnitude slower, and the gap is attributable entirely to integer representation---verified limb-based arithmetic over the primitive machine integers that both verified backends already support would close most of it.
...
Table~\ref{tab:comparison} positions our development relative to these prior formalisations. In terms of expressiveness, our library covers a broader collection of protocols, and all compositions are $n$-ary, instantiable at any arity at compile time. In terms of assurance for the executable code, prior developments either stop at machine-checked proofs or rely on Rocq's unverified extraction to OCaml; ours additionally offers a WebAssembly path through a verified compiler, and it is the only one targeting both the client and the server side of an application from a single set of proofs.

\begin{table}
  \caption{Comparison with prior machine-checked formalisations of $\Sigma$-protocols.}
  \label{tab:comparison}
  \footnotesize
  \setlength{\tabcolsep}{3pt}
  \begin{tabular}{@{}lllll@{}}
    \toprule
    Work & Prover & Extraction & Verified comp. & Real data\\
    \midrule
    Barthe et al.~\cite{5552642} & Rocq & -- & -- & --\\
    Butler et al.~\cite{butler2021formalising} & Isabelle/HOL & -- & -- & --\\
    Haines et al.~\cite{287095} & Rocq & OCaml & -- & IACR '20\\
    Firsov et al.~\cite{10221929} & EasyCrypt & -- & -- & --\\
    This work & Rocq & OCaml, Wasm & Wasm & IACR '23/'24\\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{acks}
Hidden for the anonymous submission ([anonymous,review] document class
options); to be completed only for the camera-ready version.
\end{acks}

\begin{ethics}
This work develops a formally verified library of $\Sigma$-protocols together
with machine-checked security proofs; it does not involve human subjects,
personal or sensitive data, or user studies, and therefore no approval from an
institutional review board or similar ethics panel was sought. The Helios
verifier case study operates exclusively on publicly available
bulletin-board data from the IACR 2023 and 2024 elections, which is published
by the IACR for the explicit purpose of independent public verification and
contains no private voter information. Our contributions are constructive and
defensive in nature: we do not disclose new vulnerabilities in deployed
systems, and the goal of the work is to improve the reliability of
privacy-preserving voting software.
\end{ethics}

\begin{openscience}
Our complete formalisation, comprising roughly 30,000 lines of Rocq proofs, is
available in anonymised form at
\url{https://anonymous.4open.science/r/SigmaProtocol-EBA5/README.md}. The
development is implemented in Rocq~9.0.1, with additional dependencies on
Dune, ExtLib, Menhir, Cryptokit, HACL$^{*}$, Lwt, Yojson, and Cohttp-lwt-unix;
detailed setup and usage instructions are provided in the
\texttt{src/README.md} file of the repository. The repository also contains
the extracted OCaml and WebAssembly artifacts, including the certified Helios
verifier used to validate the IACR 2023 and 2024 election data. Upon
acceptance, we will release a de-anonymised version of the repository under an
open-source licence.
\end{openscience}

\begin{ai}
We have used AI for proof reading. We have manually verified
and are responsible for the accuracy, originality, and integrity of the
output of all AI-based tools.
\end{ai}
...
\section{Rocq Listings}\label{appendix_listings}
This appendix collects, in order of appearance, the Rocq listings referenced from the main body of the paper.
