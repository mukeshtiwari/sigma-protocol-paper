...
%% TODO(authors): fill in the [TBD] placeholders below with actual
%% measurements before submission, and adjust the hardware/browser
%% description to match the machines used.
To assess whether the extracted code is practical, we measure both ends of the pipeline: the WebAssembly voting client in the browser and the OCaml Helios verifier on real election data. All measurements were taken on [TBD: hardware, OS, browser version, OCaml version]; we report the median of [TBD] runs.

\begin{table}
  \caption{Performance of the extracted code. Client-side numbers are for a single ballot with $n$ candidates in the browser; verifier numbers are for complete IACR election data sets.}
  \label{tab:perf}
  \small
  \begin{tabular}{@{}llr@{}}
    \toprule
    ...
    \midrule
    Ballot encryption + proofs (Wasm) & $n = 7$ candidates & [TBD]\\
    Ballot encryption + proofs (Helios JS) & $n = 7$ candidates & [TBD]\\
    Ballot verification (OCaml) & single ballot & [TBD]\\
    Helios verifier, IACR 2023 (OCaml) & [TBD] ballots & [TBD]\\
    Helios verifier, IACR 2024 (OCaml) & [TBD] ballots & [TBD]\\
    \bottomrule
  \end{tabular}
\end{table}

Table~\ref{tab:perf} summarises the results. For the voting client, the relevant metric is not raw throughput but perceived latency: a voter encrypts a single ballot, so encryption and proof generation must complete within interactive time bounds on commodity hardware. We compare against the hand-written JavaScript of the Helios voting booth on identical parameters. For the verifier, the relevant metric is the end-to-end wall-clock time to validate an entire election, which is an offline batch task run by scrutineers. Our arithmetic is performed on Rocq's arbitrary-precision integers rather than a specialised cryptographic library, so we do not expect to match the throughput of hand-optimised implementations such as HACL$^{*}$~\cite{10.1145/3133956.3134043}; the measurements show that the certified code is nonetheless [TBD: e.g., ``well within interactive latency budgets for ballot preparation, and verifies a complete IACR election in under a minute''].
